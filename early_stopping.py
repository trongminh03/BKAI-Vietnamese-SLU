...
class EarlyStopping:
    ...
    def __init__(self, patience=7, verbose=False, ema=None):
        """
        Args:
    ...
    def __call__(self, val_loss, model, args):
        ...
        if self.best_score is None:
            ...
            self.save_checkpoint_ema(val_loss, model, args)
    ...
    def save_checkpoint_ema(self, val_loss, model, args):
        ...
        model_dir_ema = args.model_dir.split("/") 
        model_dir_ema[0] += "_ema" 
        model_dir_ema = "/".join(model_dir_ema)
        with self.ema.average_parameters(): 
            model.save_pretrained(model_dir_ema)
        torch.save(args, os.path.join(model_dir_ema, "training_args.bin")) 
        self.val_loss_min = val_loss
